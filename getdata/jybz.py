...
def operatorjybz():
    #公司名称、统一社会信用代码、性质、授权查看人员、受权查看部门 、编号、届次、任期开始时间、任期结束时间、经营班子成员、状态、届次时间、角色信息、推荐方、届次结束时间、推荐函
    temp = []
    last = []
    opertor = r'restjybz.txt'
    ...
    for i in range(count):
        ...
        if "公司名称" in result and "<value>" in index_common.read(operator=opertor, line=i + 1):
            ...
            if len(endDate) == 11:
                ...
                temp.append('')
        elif "经营班子成员" in result and "<value>" in index_common.read(operator=opertor, line=i + 1):
            manager = re.split(r"\[|\]|\>|\<", index_common.read(operator=opertor, line=i + 1).strip())
            # print(manager)
            # print(len(manager))
            if len(manager) == 11:
                temp.append(manager[5])
            else:
                temp.append('')
...
def insertjybz():
    ...
    cur = db.cursor()
    # 插入到restdsh
    get_jybz = operatorjybz()

    # 备份数据库，备份上一次数据库
    try:
        if index_common.table_exists(cur, 'gkrestjybz.bak') == 1:
            cur.execute('drop table if exists `gkrestjybz.bak`  ')  # 在插入之前清除数据库
            cur.execute("CREATE TABLE if not exists `gkrestjybz.bak` SELECT * FROM `gkrestjybz`; ")
            cur.execute("ALTER TABLE `gkrestjybz.bak` ADD PRIMARY KEY(`jybz_id`); ")  # 设置主键
        else:
            # 删除表之前先备份
            cur.execute("CREATE TABLE if not exists `gkrestjybz.bak` SELECT * FROM `gkrestjybz`; ")
            cur.execute("ALTER TABLE `gkrestjybz.bak` ADD PRIMARY KEY(`jybz_id`); ")  # 设置主键
    ...
    # 入数据之前清空数据
    cur.execute('DELETE FROM `gkrestjybz` WHERE 1')  # 在插入之前清除数据库
    ...
    for i in get_jybz:
        ...
        # print(i)
        cur.execute('INSERT INTO `gkrestjybz`  VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',i)
    #多条插入
    # cur.executemany('INSERT INTO `restdsh`  VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',get_jybz)


    cur.connection.commit()  # 执行commit操作，插入语句才能生效
    print("jybz insert ok")
if __name__ == '__main__':
    #操作董事会
    # get_dsh = operatorjybz()
    # print(get_dsh[0:3])
    insertjybz()